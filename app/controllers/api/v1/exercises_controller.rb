class Api::V1::ExercisesController < ApplicationController
  def find_queried_exercises
    ...
    starting_index = exercise_params[:page_num] * 10
    if starting_index <= filtered_by_muscle_groups.length
      end_index = starting_index + 9
      filtered_by_muscle_groups = filtered_by_muscle_groups[starting_index..end_index]
    else
      filtered_by_muscle_groups = []
    end

    if !filtered_by_muscle_groups.empty? && filtered_by_muscle_groups
